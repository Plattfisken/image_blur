...
async def blur_rect(application_name: str, application_guid: str, rect_file: UploadFile = File(...), image_file: UploadFile = File(...)):
    ...
    file_extension = os.path.splitext(image_file.filename or "")[1]
    try:
    ...
    blurred_img.save(return_file, format)
    return_file.seek(0)

    media_type = "image/png" if format == "png" else image_file.content_type
    return StreamingResponse(return_file, media_type=media_type)
